...
for NS in range(ins+1):
	print (NS,ins,plot_south)
	lat_mult = 1
	if NS == 1: lat_mult = -1
...
# 		  If running NOWCAST Mode

	ifl = 	'Ovation_Latest_aurora_N_'
	if NS == 1: ifl = 'Ovation_Latest_aurora_S_'

# 		if run_realtime == False:

# 	ifl = 'North/aurora_N_' + file_date
# 	if NS == 1:
#  			ifl = 'South/aurora_S_' + file_date
# 	start_hour = 0
...
# 		print ('data date  ',fdatei)
	...
	hour = int(fdate2[0])
	minute = int(fdate2[1])
	fdate = dt.datetime(year, month, day, hour, minute)


	file_date = str(myear) + '-' + str(mmonth) + '-' + str(mday) + '_' + str(mhour) + str(mminute)
...
#    *********************   Output Hemispheric Power   *****************
	Date_string = "%4i%2.2i%2.2i"%(myear,mmonth,mday)
	Date_string2 = "%4i-%2.2i-%2.2i"%(myear,mmonth,mday)
	Date_Time_string = "%4i-%2.2i-%2.2i %2.2i:%2.2i"%(myear,mmonth,mday,mhour,mminute)
	HP_str = "    %4i"%(HP)
	HP_file = Output_path + "/Power/SWPC_aurora_power_" + Date_string + ".txt"

	if path.exists(HP_file) == False:
		with open(Home_path + 'HP_Header/HP_Header_1.txt') as h1:
			hd1 = h1.read()
		with open(Home_path + 'HP_Header/HP_Header_2.txt') as h2:
			hd2 = h2.read()
		hd1 = hd1[0:len(hd1)-1]
		hd1 += Date_string2 + "\n"
		hd1 += hd2
		# Create dir if missing
		os.makedirs(Output_path + '/Power', exist_ok=True)
...
#    ******************   Output Data to ASCII Array  *********************
	if output_global_ascii == True:
		...
		if NS ==1:
			...
			ofilet = Output_path+ "/Grid/Aurora_30-min_" + file_date + ".txt"
			os.makedirs(Output_path + "/Grid", exist_ok=True)
...
# 	ofile1 = Output_path + ifl

#
#
	ofile = Output_path+ "/North/Aurora_30-min_N_" + file_date + ".png"
	if NS == 1: ofile = Output_path+ "/South/Aurora_30-min_S_" + file_date + ".png"
# 		print (ofile)
# #
# 		im.save(ofile)
	...
	icon = Image.open(Home_path + "HP_Header/NOAA_logo4.png")
	icon_size = (65,65)
	...
	# Create dir if missing
	os.makedirs(Output_path+ '/North', exist_ok=True)
	os.makedirs(Output_path + '/South', exist_ok=True)
	plt.savefig(ofile ,facecolor = 'black', edgecolor = 'black',bbox_inches = 'tight', pad_inches = 0)
