...
#  This code reads the output text file from the OVATION 3-Day forecast model and creates 
#  maps of the aurora with a North polar perspective.  
...
Input_path = os.environ.get('Input_path','/home/rodney.viereck/python/ovation/Output/FORECAST/Text/North/')
Image_Output_path = os.environ.get('Output_path','/home/rodney.viereck/python/ovation/Final_Output/FORECAST/Images/')
# Text_Output_path = os.environ.get('Output_path','/home/rodney.viereck/python/ovation/Final_Output/Text/')
...
# run_once = False
# plot_south = False
# Input_path = Input_path + '/FORECAST/Text/'
...
# 		vlon = -90
	...
	input_file = open(Input_path + in_file, 'r')			
	
#  Read Input File and Parse Critical Variables
	
	mhead = input_file.readline()
...
# 		print ('data date  ',fdatei)
	...
	mhour = int(mdate2[0])
	mmin = int(mdate2[1])
# 	dec_time = float(mhour) + float(mmin)/60.
	
	mdate = dt.datetime(myear, mmonth, mday, mhour, mmin)    #Time Forecast was made
	...
	dec_time = float(fhour) + float(fmin)/60.
		
	fdate = dt.datetime(fyear, fmonth, fday, fhour, fmin)     #Time Forecast is Valid
	...
	Kp = float(f_hp_head[2])			#Read Hemispheric Power
...
#  Set up lat-lon arrays and fill them
	...
	geo_aur = 1.1 * gaussian_filter(geo_aur,sigma = 6,mode = 'wrap')		#Smooth data to remove artifacts from geomag to geograph transform		
...
# 		plt.figtext(0.99,0.99,'Forecast Lead Time: %3i minutes' %int(lt), color = 'yellow', size =12, ha = 'right')
	...
	plt.figtext(0.99,0.04,'OVATION Aurora Model', color = text_color, size =18, ha = 'right')
	plt.figtext(0.99,0.02, 'Model Run at %s (UTC)' % now.strftime("%Y-%m-%d %H:%M"),color=text_color,size = 12,ha = 'right')
	plt.figtext(0.99,0.00, 'Kp Forecast Made at %s (UTC)' % mdate.strftime("%Y-%m-%d %H:%M"),color=text_color,size = 12,ha = 'right')
...
# 	if run_realtime == True:	
# 		ofile_image = Image_Output_path + out_file 
